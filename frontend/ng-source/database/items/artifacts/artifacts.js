angular.module('database.items.artifacts', [])
    .config(['$stateProvider', function ($stateProvider) {
        $stateProvider
            .state('app.database.items.artifacts', {
                url: '/artifacts',
                templateUrl: 'database/items/artifacts/artifacts-list.html',
                controller: 'ArtifactCtrl',
                ncyBreadcrumb: {
                    label: 'Artifacts'
                },
                data: {
                    requiredPermission: 'database.items.artifacts.view'
                }
            })

            .state('app.database.items.artifacts.view', {
                url: '/:id',
                templateUrl: 'database/items/artifacts/artifacts-view.html',
    ...
    .controller('ArtifactCtrl', ['$scope', '$rootScope', '$timeout', 'ItemArtifactService', 'HistoryService', 'FilterService', 'DatabaseService', 'ItemsDatabaseService', 'ItemsAdministrationService', 'ListService',
        function ($scope, $rootScope, $timeout, ItemArtifactService, HistoryService, FilterService, DatabaseService, ItemsDatabaseService, ItemsAdministrationService, ListService) {
            $scope.activeFilters = HistoryService.loadOrCreate('database.items.artifacts.filters', {});
            $scope.searchSettings = HistoryService.loadOrCreate('database.items.artifacts.searchSettings', {
                elementsPerPage: 10,
            ...
            $scope.simpleFilters = HistoryService.loadOrCreate('database.items.artifacts.simpleFilters', {
                nameContains: ''
            });

            // Recalls data that came from the last page's visit
            $scope.data = HistoryService.loadOrCreate('database.items.artifacts.data', null);
            $scope.metadata = HistoryService.loadOrCreate('database.items.artifacts.metadata', null);
            ...
            $scope.$watch('searchSettings', function (newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.artifacts.searchSettings', $scope.searchSettings);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('activeFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.artifacts.filters', $scope.activeFilters);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('simpleFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.artifacts.simpleFilters', $scope.simpleFilters);
                }
            }, true);
            ...
            $scope.refreshList = function () {
                ...
                ListService.refreshList(sendToServer, ItemArtifactService).then(function(response) {
                    // Fills in data
                    $scope.data = response.data;
                    $scope.metadata = response.metadata;

                    // Saves data
                    HistoryService.save('database.items.artifacts.data', $scope.data);
                    HistoryService.save('database.items.artifacts.metadata', $scope.metadata);
            ...
            $scope.postProcessRefresh = function() {
                for (var groupIdx in $scope.data) {
                    for (var dataIdx in $scope.data[groupIdx].data) {
                        ...
                        // Stats' HTML
                        $scope.data[groupIdx].data[dataIdx].StatsHTML = ItemArtifactService.constructStatsHTML($scope.data[groupIdx].data[dataIdx]);
            ...
            $scope.deleteItem = function (item) {
                ItemsAdministrationService.deleteItem(item.ID, ItemArtifactService).then(function() {
                    item.deleted_at = new Date();
                });
            };

            $scope.restoreItem = function (item) {
                ItemsAdministrationService.restoreItem(item.ID, ItemArtifactService).then(function() {
                    item.deleted_at = null;
                });
            }
        }])

    .controller('ArtifactViewCtrl', ['$scope', '$stateParams', 'ItemArtifactService', 'ItemsDatabaseService',
        function ($scope, $stateParams, ItemArtifactService, ItemsDatabaseService) {
            ...
            $scope.postProcessRefresh = function() {
                ...
                // Stats' HTML
                $scope.data.StatsHTML = ItemArtifactService.constructStatsHTML($scope.data);
    ...
    .factory('ItemArtifactService', ['$http',
        function ($http) {
            return {
                /** Gets a shortened listing of available artifacts. */
                ...
                /** Gets a single detailed artifact entry */
                get: function (id) {
                    return $http
                        .get('/artifacts/' + id);
                },
                ...
                constructStatsHTML: function(item) {
                    ...
                    if (item.CRTATK != 0)
                        additionalStatsHTML += '<li>' + item.CRTATK + '% Critical Damage</li>';
                    if (item.DEF != 0)
                    ...
                    if (item.SummonAR != 0)
                        additionalStatsHTML += '<li>' + item.SummonAR + ' Summon AR</li>';
                    if (item.SummonDR != 0)
                        additionalStatsHTML += '<li>' + item.SummonDR + ' Summon DR</li>';
                    if (item.AddSummonMHP != 0)
                        additionalStatsHTML += '<li>' + item.AddSummonMHP + ' Summon Max HP</li>';
