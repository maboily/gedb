angular.module('database.items.belts', [])
    .config(['$stateProvider', function ($stateProvider) {
        $stateProvider
            .state('app.database.items.belts', {
                url: '/belts',
                templateUrl: 'database/items/belts/belts-list.html',
                controller: 'BeltCtrl',
                ncyBreadcrumb: {
                    label: 'Belts'
                },
                data: {
                    requiredPermission: 'database.items.belts.view'
                }
            })

            .state('app.database.items.belts.view', {
                url: '/:id',
                templateUrl: 'database/items/belts/belts-view.html',
                controller: 'BeltViewCtrl',
                ncyBreadcrumb: {
                    label: 'View Belt'
                },
                data: {
    ...
    .controller('BeltCtrl', ['$scope', '$rootScope', '$timeout', 'ItemBeltService', 'HistoryService', 'FilterService', 'DatabaseService', 'ItemsDatabaseService', 'ItemsAdministrationService', 'ListService',
        function ($scope, $rootScope, $timeout, ItemBeltService, HistoryService, FilterService, DatabaseService, ItemsDatabaseService, ItemsAdministrationService, ListService) {
            $scope.activeFilters = HistoryService.loadOrCreate('database.items.belts.filters', {});
            $scope.searchSettings = HistoryService.loadOrCreate('database.items.belts.searchSettings', {
                elementsPerPage: 10,
            ...
            $scope.simpleFilters = HistoryService.loadOrCreate('database.items.belts.simpleFilters', {
                nameContains: '',
                beltType: ''
            });

            // Recalls data that came from the last page's visit
            $scope.data = HistoryService.loadOrCreate('database.items.belts.data', null);
            $scope.metadata = HistoryService.loadOrCreate('database.items.belts.metadata', null);
            ...
            $scope.$watch('searchSettings', function (newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.belts.searchSettings', $scope.searchSettings);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('activeFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.belts.filters', $scope.activeFilters);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('simpleFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.belts.simpleFilters', $scope.simpleFilters);
                }
            }, true);
            ...
            $scope.recompileSimpleFilters = function () {
                ...
                if ($scope.simpleFilters.beltType != '')
                    filters.push(FilterService.createFilter('ClassTypeQualifier', 'eq', $scope.simpleFilters.beltType));
            ...
            $scope.refreshList = function () {
                ...
                ListService.refreshList(sendToServer, ItemBeltService).then(function(response) {
                    // Fills in data
                    $scope.data = response.data;
                    $scope.metadata = response.metadata;

                    // Saves data
                    HistoryService.save('database.items.belts.data', $scope.data);
                    HistoryService.save('database.items.belts.metadata', $scope.metadata);
            ...
            $scope.postProcessRefresh = function() {
                for (var groupIdx in $scope.data) {
                    for (var dataIdx in $scope.data[groupIdx].data) {
                        ...
                        // Stats' HTML
                        $scope.data[groupIdx].data[dataIdx].StatsHTML = ItemBeltService.constructStatsHTML($scope.data[groupIdx].data[dataIdx]);
            ...
            $scope.deleteItem = function (item) {
                ItemsAdministrationService.deleteItem(item.ID, ItemBeltService).then(function() {
                    item.deleted_at = new Date();
                });
            };

            $scope.restoreItem = function (item) {
                ItemsAdministrationService.restoreItem(item.ID, ItemBeltService).then(function() {
                    item.deleted_at = null;
                });
            }
        }])

    .controller('BeltViewCtrl', ['$scope', '$stateParams', 'ItemBeltService', 'ItemsDatabaseService',
        function ($scope, $stateParams, ItemBeltService, ItemsDatabaseService) {
            ...
            $scope.postProcessRefresh = function() {
                ...
                // Stats' HTML
                $scope.data.StatsHTML = ItemBeltService.constructStatsHTML($scope.data);
    ...
    .factory('ItemBeltService', ['$http',
        function ($http) {
            return {
                /** Gets a shortened listing of available belts. */
                ...
                /** Gets a single detailed belt entry */
                get: function (id) {
                    return $http
                        .get('/belts/' + id);
                },
