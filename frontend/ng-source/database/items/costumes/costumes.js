angular.module('database.items.costumes', [])
    .config(['$stateProvider', function ($stateProvider) {
        $stateProvider
            .state('app.database.items.costumes', {
                url: '/costumes',
                templateUrl: 'database/items/costumes/costumes-list.html',
                controller: 'CostumeCtrl',
                ncyBreadcrumb: {
                    label: 'Costumes'
                },
                data: {
                    requiredPermission: 'database.items.costumes.view'
                }
            })

            .state('app.database.items.costumes.view', {
                url: '/:id',
                templateUrl: 'database/items/costumes/costumes-view.html',
                controller: 'CostumeViewCtrl',
                ncyBreadcrumb: {
                    label: 'View Costume'
                },
                data: {
    ...
    .controller('CostumeCtrl', ['$scope', '$rootScope', '$timeout', 'ItemCostumeService', 'HistoryService', 'FilterService', 'DatabaseService', 'ItemsDatabaseService', 'ItemsAdministrationService', 'ListService',
        function ($scope, $rootScope, $timeout, ItemCostumeService, HistoryService, FilterService, DatabaseService, ItemsDatabaseService, ItemsAdministrationService, ListService) {
            $scope.activeFilters = HistoryService.loadOrCreate('database.items.costumes.filters', {});
            $scope.searchSettings = HistoryService.loadOrCreate('database.items.costumes.searchSettings', {
                elementsPerPage: 10,
            ...
            $scope.simpleFilters = HistoryService.loadOrCreate('database.items.costumes.simpleFilters', {
                nameContains: '',
                costumeType: ''
            });

            // Recalls data that came from the last page's visit
            $scope.data = HistoryService.loadOrCreate('database.items.costumes.data', null);
            $scope.metadata = HistoryService.loadOrCreate('database.items.costumes.metadata', null);
            ...
            $scope.$watch('searchSettings', function (newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.costumes.searchSettings', $scope.searchSettings);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('activeFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.costumes.filters', $scope.activeFilters);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('simpleFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.costumes.simpleFilters', $scope.simpleFilters);
                }
            }, true);
            ...
            $scope.recompileSimpleFilters = function () {
                ...
                if ($scope.simpleFilters.costumeType != '')
                    filters.push(FilterService.createFilter('ClassTypeQualifier', 'eq', $scope.simpleFilters.costumeType));
            ...
            $scope.refreshList = function () {
                ...
                ListService.refreshList(sendToServer, ItemCostumeService).then(function(response) {
                    // Fills in data
                    $scope.data = response.data;
                    $scope.metadata = response.metadata;

                    // Saves data
                    HistoryService.save('database.items.costumes.data', $scope.data);
                    HistoryService.save('database.items.costumes.metadata', $scope.metadata);
            ...
            $scope.postProcessRefresh = function() {
                for (var groupIdx in $scope.data) {
                    for (var dataIdx in $scope.data[groupIdx].data) {
                        ...
                        // Stats' HTML
                        $scope.data[groupIdx].data[dataIdx].StatsHTML = ItemCostumeService.constructStatsHTML($scope.data[groupIdx].data[dataIdx]);
            ...
            $scope.deleteItem = function (item) {
                ItemsAdministrationService.deleteItem(item.ID, ItemCostumeService).then(function() {
                    item.deleted_at = new Date();
                });
            };

            $scope.restoreItem = function (item) {
                ItemsAdministrationService.restoreItem(item.ID, ItemCostumeService).then(function() {
                    item.deleted_at = null;
                });
            }
        }])

    .controller('CostumeViewCtrl', ['$scope', '$stateParams', 'ItemCostumeService', 'ItemsDatabaseService',
        function ($scope, $stateParams, ItemCostumeService, ItemsDatabaseService) {
            ...
            $scope.postProcessRefresh = function() {
                ...
                // Stats' HTML
                $scope.data.StatsHTML = ItemCostumeService.constructStatsHTML($scope.data);
    ...
    .factory('ItemCostumeService', ['$http',
        function ($http) {
            return {
                /** Gets a shortened listing of available costumes. */
                ...
                /** Gets a single detailed costume entry */
                get: function (id) {
                    return $http
                        .get('/costumes/' + id);
                },
