angular.module('database.items.gloves', [])
    .config(['$stateProvider', function ($stateProvider) {
        $stateProvider
            .state('app.database.items.gloves', {
                url: '/gloves',
                templateUrl: 'database/items/gloves/gloves-list.html',
                controller: 'GloveCtrl',
                ncyBreadcrumb: {
                    label: 'Gloves'
                },
                data: {
                    requiredPermission: 'database.items.gloves.view'
                }
            })

            .state('app.database.items.gloves.view', {
                url: '/:id',
                templateUrl: 'database/items/gloves/gloves-view.html',
                controller: 'GloveViewCtrl',
                ncyBreadcrumb: {
                    label: 'View Glove'
                },
                data: {
    ...
    .controller('GloveCtrl', ['$scope', '$rootScope', '$timeout', 'ItemGloveService', 'HistoryService', 'FilterService', 'DatabaseService', 'ItemsDatabaseService', 'ItemsAdministrationService', 'ListService',
        function ($scope, $rootScope, $timeout, ItemGloveService, HistoryService, FilterService, DatabaseService, ItemsDatabaseService, ItemsAdministrationService, ListService) {
            $scope.activeFilters = HistoryService.loadOrCreate('database.items.gloves.filters', {});
            $scope.searchSettings = HistoryService.loadOrCreate('database.items.gloves.searchSettings', {
                elementsPerPage: 10,
            ...
            $scope.simpleFilters = HistoryService.loadOrCreate('database.items.gloves.simpleFilters', {
                nameContains: '',
                gloveType: ''
            });

            // Recalls data that came from the last page's visit
            $scope.data = HistoryService.loadOrCreate('database.items.gloves.data', null);
            $scope.metadata = HistoryService.loadOrCreate('database.items.gloves.metadata', null);
            ...
            $scope.$watch('searchSettings', function (newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.gloves.searchSettings', $scope.searchSettings);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('activeFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.gloves.filters', $scope.activeFilters);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('simpleFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.gloves.simpleFilters', $scope.simpleFilters);
                }
            }, true);
            ...
            $scope.recompileSimpleFilters = function () {
                ...
                if ($scope.simpleFilters.gloveType != '')
                    filters.push(FilterService.createFilter('ClassTypeQualifier', 'eq', $scope.simpleFilters.gloveType));
            ...
            $scope.refreshList = function () {
                ...
                ListService.refreshList(sendToServer, ItemGloveService).then(function(response) {
                    // Fills in data
                    $scope.data = response.data;
                    $scope.metadata = response.metadata;

                    // Saves data
                    HistoryService.save('database.items.gloves.data', $scope.data);
                    HistoryService.save('database.items.gloves.metadata', $scope.metadata);
            ...
            $scope.postProcessRefresh = function() {
                for (var groupIdx in $scope.data) {
                    for (var dataIdx in $scope.data[groupIdx].data) {
                        ...
                        // Stats' HTML
                        $scope.data[groupIdx].data[dataIdx].StatsHTML = ItemGloveService.constructStatsHTML($scope.data[groupIdx].data[dataIdx]);
            ...
            $scope.deleteItem = function (item) {
                ItemsAdministrationService.deleteItem(item.ID, ItemGloveService).then(function() {
                    item.deleted_at = new Date();
                });
            };

            $scope.restoreItem = function (item) {
                ItemsAdministrationService.restoreItem(item.ID, ItemGloveService).then(function() {
                    item.deleted_at = null;
                });
            }
        }])

    .controller('GloveViewCtrl', ['$scope', '$stateParams', 'ItemGloveService', 'ItemsDatabaseService',
        function ($scope, $stateParams, ItemGloveService, ItemsDatabaseService) {
            ...
            $scope.postProcessRefresh = function() {
                ...
                // Stats' HTML
                $scope.data.StatsHTML = ItemGloveService.constructStatsHTML($scope.data);
    ...
    .factory('ItemGloveService', ['$http',
        function ($http) {
            return {
                /** Gets a shortened listing of available gloves. */
                ...
                /** Gets a single detailed glove entry */
                get: function (id) {
                    return $http
                        .get('/gloves/' + id);
                },
