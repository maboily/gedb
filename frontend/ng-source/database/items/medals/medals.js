angular.module('database.items.medals', [])
    .config(['$stateProvider', function ($stateProvider) {
        $stateProvider
            .state('app.database.items.medals', {
                url: '/medals',
                templateUrl: 'database/items/medals/medals-list.html',
                controller: 'MedalCtrl',
                ncyBreadcrumb: {
                    label: 'Medals'
                },
                data: {
                    requiredPermission: 'database.items.medals.view'
                }
            })

            .state('app.database.items.medals.view', {
                url: '/:id',
                templateUrl: 'database/items/medals/medals-view.html',
                controller: 'MedalViewCtrl',
                ncyBreadcrumb: {
                    label: 'View Medal'
                },
                data: {
    ...
    .controller('MedalCtrl', ['$scope', '$rootScope', '$timeout', 'ItemMedalService', 'HistoryService', 'FilterService', 'DatabaseService', 'ItemsDatabaseService', 'ItemsAdministrationService', 'ListService',
        function ($scope, $rootScope, $timeout, ItemMedalService, HistoryService, FilterService, DatabaseService, ItemsDatabaseService, ItemsAdministrationService, ListService) {
            $scope.activeFilters = HistoryService.loadOrCreate('database.items.medals.filters', {});
            $scope.searchSettings = HistoryService.loadOrCreate('database.items.medals.searchSettings', {
                elementsPerPage: 25,
            ...
            $scope.simpleFilters = HistoryService.loadOrCreate('database.items.medals.simpleFilters', {
                nameContains: ''
            });

            // Recalls data that came from the last page's visit
            $scope.data = HistoryService.loadOrCreate('database.items.medals.data', null);
            $scope.metadata = HistoryService.loadOrCreate('database.items.medals.metadata', null);
            ...
            $scope.$watch('searchSettings', function (newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.medals.searchSettings', $scope.searchSettings);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('activeFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.medals.filters', $scope.activeFilters);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('simpleFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.medals.simpleFilters', $scope.simpleFilters);
                }
            }, true);
            ...
            $scope.refreshList = function () {
                ...
                ListService.refreshList(sendToServer, ItemMedalService).then(function(response) {
                    // Fills in data
                    $scope.data = response.data;
                    $scope.metadata = response.metadata;

                    // Saves data
                    HistoryService.save('database.items.medals.data', $scope.data);
                    HistoryService.save('database.items.medals.metadata', $scope.metadata);
            ...
            $scope.postProcessRefresh = function() {
                for (var groupIdx in $scope.data) {
                    for (var dataIdx in $scope.data[groupIdx].data) {
                        ...
                        // Stats' HTML
                        $scope.data[groupIdx].data[dataIdx].StatsHTML = ItemMedalService.constructStatsHTML($scope.data[groupIdx].data[dataIdx]);
            ...
            $scope.deleteItem = function (item) {
                ItemsAdministrationService.deleteItem(item.ID, ItemMedalService).then(function() {
                    item.deleted_at = new Date();
                });
            };

            $scope.restoreItem = function (item) {
                ItemsAdministrationService.restoreItem(item.ID, ItemMedalService).then(function() {
                    item.deleted_at = null;
                });
            }
        }])

    .controller('MedalViewCtrl', ['$scope', '$stateParams', 'ItemMedalService', 'ItemsDatabaseService',
        function ($scope, $stateParams, ItemMedalService, ItemsDatabaseService) {
            ...
            $scope.postProcessRefresh = function() {
                ...
                // Stats' HTML
                $scope.data.StatsHTML = ItemMedalService.constructStatsHTML($scope.data);
    ...
    .factory('ItemMedalService', ['$http',
        function ($http) {
            return {
                /** Gets a shortened listing of available medals. */
                ...
                /** Gets a single detailed medal entry */
                get: function (id) {
                    return $http
                        .get('/medals/' + id);
                },
