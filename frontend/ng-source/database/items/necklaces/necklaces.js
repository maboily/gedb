angular.module('database.items.necklaces', [])
    .config(['$stateProvider', function ($stateProvider) {
        $stateProvider
            .state('app.database.items.necklaces', {
                url: '/necklaces',
                templateUrl: 'database/items/necklaces/necklaces-list.html',
                controller: 'NecklaceCtrl',
                ncyBreadcrumb: {
                    label: 'Necklaces'
                },
                data: {
                    requiredPermission: 'database.items.necklaces.view'
                }
            })

            .state('app.database.items.necklaces.view', {
                url: '/:id',
                templateUrl: 'database/items/necklaces/necklaces-view.html',
                controller: 'NecklaceViewCtrl',
                ncyBreadcrumb: {
                    label: 'View Necklace'
                },
                data: {
    ...
    .controller('NecklaceCtrl', ['$scope', '$rootScope', '$timeout', 'ItemNecklaceService', 'HistoryService', 'FilterService', 'DatabaseService', 'ItemsDatabaseService', 'ItemsAdministrationService', 'ListService',
        function ($scope, $rootScope, $timeout, ItemNecklaceService, HistoryService, FilterService, DatabaseService, ItemsDatabaseService, ItemsAdministrationService, ListService) {
            $scope.activeFilters = HistoryService.loadOrCreate('database.items.necklaces.filters', {});
            $scope.searchSettings = HistoryService.loadOrCreate('database.items.necklaces.searchSettings', {
                elementsPerPage: 10,
            ...
            $scope.simpleFilters = HistoryService.loadOrCreate('database.items.necklaces.simpleFilters', {
                nameContains: ''
            });

            // Recalls data that came from the last page's visit
            $scope.data = HistoryService.loadOrCreate('database.items.necklaces.data', null);
            $scope.metadata = HistoryService.loadOrCreate('database.items.necklaces.metadata', null);
            ...
            $scope.$watch('searchSettings', function (newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.necklaces.searchSettings', $scope.searchSettings);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('activeFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.necklaces.filters', $scope.activeFilters);
                    $scope.pendRefresh();
                }
            }, true);
            ...
            $scope.$watch('simpleFilters', function(newValue, oldValue) {
                if (newValue !== oldValue || $scope.data == null) {
                    HistoryService.save('database.items.necklaces.simpleFilters', $scope.simpleFilters);
                }
            }, true);
            ...
            $scope.refreshList = function () {
                ...
                ListService.refreshList(sendToServer, ItemNecklaceService).then(function(response) {
                    // Fills in data
                    $scope.data = response.data;
                    $scope.metadata = response.metadata;

                    // Saves data
                    HistoryService.save('database.items.necklaces.data', $scope.data);
                    HistoryService.save('database.items.necklaces.metadata', $scope.metadata);
            ...
            $scope.postProcessRefresh = function() {
                for (var groupIdx in $scope.data) {
                    for (var dataIdx in $scope.data[groupIdx].data) {
                        ...
                        // Stats' HTML
                        $scope.data[groupIdx].data[dataIdx].StatsHTML = ItemNecklaceService.constructStatsHTML($scope.data[groupIdx].data[dataIdx]);
            ...
            $scope.deleteItem = function (item) {
                ItemsAdministrationService.deleteItem(item.ID, ItemNecklaceService).then(function() {
                    item.deleted_at = new Date();
                });
            };

            $scope.restoreItem = function (item) {
                ItemsAdministrationService.restoreItem(item.ID, ItemNecklaceService).then(function() {
                    item.deleted_at = null;
                });
            }
        }])

    .controller('NecklaceViewCtrl', ['$scope', '$stateParams', 'ItemNecklaceService', 'ItemsDatabaseService',
        function ($scope, $stateParams, ItemNecklaceService, ItemsDatabaseService) {
            ...
            $scope.postProcessRefresh = function() {
                ...
                // Stats' HTML
                $scope.data.StatsHTML = ItemNecklaceService.constructStatsHTML($scope.data);
    ...
    .factory('ItemNecklaceService', ['$http',
        function ($http) {
            return {
                /** Gets a shortened listing of available necklaces. */
                ...
                /** Gets a single detailed necklace entry */
                get: function (id) {
                    return $http
                        .get('/necklaces/' + id);
                },
